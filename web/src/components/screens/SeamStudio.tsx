...
export function SeamStudio({ workspaceId }: { workspaceId: string }) {
  ...
  const enrich = useJob<SeamsEnrichResult>(
    () => api.startSeamsEnrich(workspaceId, true),
  ...
  const run = async () => {
    ...
    try {
      const result = await api.runSeams(workspaceId);
      setRows(result.candidates);
      setQualityPassed(typeof result.quality_passed === "boolean" ? result.quality_passed : null);
    } catch (e) {
    ...
    <div className="p-4 space-y-4 max-w-4xl">
      ...
      <div className="flex items-center gap-2">
        <button onClick={run} disabled={busy}
          ...
          <Play className="w-4 h-4" />{busy ? "Ranking…" : rows ? "Re-rank seams" : "Find seams"}
        </button>
        {rows && (
      ...
      {rows && (
        <div className="space-y-2">
          {typeof qualityPassed === "boolean" && (
            <div className={qualityPassed ? "text-xs text-emerald-400" : "text-xs text-amber-400"}>
              {qualityPassed ? "quality passed" : "quality needs review"}
            </div>
          )}
