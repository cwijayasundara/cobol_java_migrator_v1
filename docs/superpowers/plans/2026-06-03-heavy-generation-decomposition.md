...
**Env knobs (new, documented in code):** `BACKLOG_EPIC_TIMEOUT_S` (default 120), `BACKLOG_STORY_TIMEOUT_S` (default 120), `BACKLOG_MAX_CONCURRENCY` (default 4), `BACKLOG_STORY_MAX_TURNS` (default 6), `BACKLOG_GEN_MODE` (`auto`|`decomposed`|`oneshot`, default `auto` — the Classify-and-Act size router), `BACKLOG_DECOMPOSE_MIN_EPICS` (default 3 — below this, one call is fine), `BACKLOG_MAX_ROUNDS` (default 3 — the Loop-Until-Done completeness safety bound). Reuse existing `BACKLOG_TIMEOUT_S`/`BACKLOG_MAX_TURNS`/`BACKLOG_MODEL` where they still apply. **No `*_MAX_REFS` knob exists — evidence is scoped by relevance, never capped.** (Timeouts/round-bound are hung-call/runaway safety only, not output limits — the completeness loop stops on coverage-complete or no-progress, not on hitting `BACKLOG_MAX_ROUNDS` in the normal case.)
...
**Workflow pattern (per "A harness for every task"):** this orchestrator is **Fan-Out-and-Synthesize** (epics define the units → fan out per-epic story calls in isolated context windows → synthesize via merge + the deterministic DAG), wrapped in a **Loop-Until-Done** completeness guard (Step 3), and selected by a **Classify-and-Act** size router (Step 2). Do NOT add Tournament / Adversarial-Verify / Generate-and-Filter LLM layers — grounding (`_ground`) + the coverage gate already provide deterministic correctness/filtering.
...
- [ ] **Step 2: Implement the Fan-Out-and-Synthesize core + size router.** `generate_backlog_payload`: `epics_res = await generate_epics(...)`; if not `ok` → return typed failure; then `await asyncio.gather(*[generate_stories_for_epic(epic) under a semaphore(BACKLOG_MAX_CONCURRENCY)])`, each epic scoped to its own relevant refs (Task 4); merge epics + all stories into `{"epics", "stories"}`; return the merged dict (or typed failure when zero stories). Per-unit timeouts from `BACKLOG_EPIC_TIMEOUT_S`/`BACKLOG_STORY_TIMEOUT_S`. **Size router (Classify-and-Act):** pick the path by `len(brd_sections)`/epic count — tiny inputs may use a single `_generate_backlog_oneshot(...)` (the preserved legacy body) since fan-out overhead isn't worth it; larger inputs use the decomposed path. Threshold via `BACKLOG_DECOMPOSE_MIN_EPICS` (default e.g. 3). An explicit `BACKLOG_GEN_MODE=auto|decomposed|oneshot` (default `auto`) overrides the router for debug — mirroring `CODEGEN_MODE`. Keep `_generate_backlog_oneshot` reachable, NOT deleted.

- [ ] **Step 3: Loop-Until-Done completeness guard (honors the no-drop requirement).** After the first fan-out+merge, compute which BRD requirement ids are NOT yet covered by any produced story (a story covers a requirement if it cites that `brd_requirement_id`). If gaps remain, run ANOTHER bounded round generating stories ONLY for the uncovered requirements (group them into a synthetic top-up unit / per-epic as appropriate), merge the new stories in, and recheck. Stop when: coverage is complete, OR a round produces zero NEW covered requirements (no progress), OR a hard `BACKLOG_MAX_ROUNDS` (default 3) safety bound is reached. This GUARANTEES completeness by construction rather than truncating — it never drops a requirement to finish; it keeps generating until everything's covered or genuinely can't be. Test: a stub where round 1 leaves a requirement uncovered and round 2 covers it → final payload covers all; a stub that never covers a requirement → stops after no-progress without infinite loop. NOTE: this is generation-side completeness; the existing `brd_logic_coverage` gate (graph-evidence coverage) in `controlplane/backlog.py` stays as the final reduce/report — do not duplicate it, reuse its notion of coverage where practical.
